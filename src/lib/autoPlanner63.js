    ...
    showRoomStructures: function (roomName, structMap) {
        ...
        _.keys(CONTROLLER_STRUCTURES).forEach(struct => {
            if (struct == "road") {
                ...
                structMap[struct].forEach(e => visual.text(structuresShape[struct], e[0], e[1] + 0.25, {
...
global.PriorityQueue = PriorityQueue//squad使用
global.NewNode = NewNode//squad使用
...
let pro={
     ...
     */
    computeManor (roomName,points,storagePos,blocked){
        pro.init()
        for(let p of points){
        ...
        _.keys(sizeMap).forEach(pos=>{
            ...
            if(currentPutAbleList.length<minPlaneCnt)return 1
            if(storagePos&&!currentPutAbleList.find(e => e.x == storagePos.x && e.y == storagePos.y)) return 2;
            ...
            if(minPlaneCnt<currentPutAbleList.length&&wallCnt>currentWallCnt&&(currentInnerPutAbleList.filter(e=>e.k>1).length>30||maxDist>5)){
                ...
                finalPos = pos
                if(storagePos){
                    centerX = storagePos.x
                    centerY = storagePos.y
                }else {
        ...
        roomManor.forEach((x,y,val)=>{
            ...
            if(val>=2){
                ...
                if(labDistance<=distance) return 4;
                let checkCnt = 0;
         ...
         */
        roomStructs.forEach((x,y,val)=>{
            if(val == 'link'||val == 'container')return 6; // 资源点的不要 放路
            if(! val instanceof String||val>-1)return 7; // 附近有建筑 ，并且不是road
        ...
        structMap["container"].forEach(e=>costs.set(e[0],e[1],255))
        ...
        structMap["container"].forEach(e=>{
            let ret = PathFinder.search(
                ...
                {
                    pos: new RoomPosition(e[0], e[1], roomName),
                    range: 1
                },
                {
        ...
        pro.dismiss()
        return{
            roomName, structMap, centerX, centerY
        }
    }
};
