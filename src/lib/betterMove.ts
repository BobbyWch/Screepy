...
// 初始化参数
const config = {
	roomNameMoreThan100: false,
	changeMove: false, // 【未启用】为creep.move增加对穿能力
	changeMoveTo: true, // 全面优化creep.moveTo，跨房移动也可以一个moveTo解决问题
	changeFindClostestByPath: false, // 【未启用】轻度修改findClosestByPath，使得默认按照ignoreCreeps寻找最短
	autoVisual: false, // 【未启用】
	enableFlee: false, // 【未启用】是否添加flee()函数，注意这会在Creep.prototype上添加官方未有键值，flee()用法见最底下module.exports处
	visual: false /*绘制路径信息*/
};
// 运行时参数
let pathClearDelay: number | undefined = 3000; // 清理相应时间内都未被再次使用的路径，同时清理死亡creep的缓存，设为undefined表示不清除缓存
let hostileCostMatrixClearDelay: number | undefined = 500; // 自动清理相应时间前创建的其他玩家房间的costMatrix
const coreLayoutRange = 3; // 核心布局半径，在离storage这个范围内频繁检查对穿（减少堵路的等待
let avoidRooms: any = Memory.avoids ? Memory.avoids : []; // 永不踏入这些房间
const avoidExits = {
	fromRoom: 'toRoom'
}; // 【未启用】单向屏蔽房间的一些出口，永不从fromRoom踏入toRoom
/** @type {{id:string, roomName:string, taskQueue:{path:MyPath, idx:number, roomName:string}[]}[]} */
let observers: any[] = []; // 如果想用ob寻路，把ob的id放这里

/***************************************
 *  局部缓存
 */
/** @type {{ [time: number]:{path:MyPath, idx:number, roomName:string}[] }} */
const obTimer: any = {}; // 【未启用】用于登记ob调用，在相应的tick查看房间对象
let obTick = Game.time;
/** @type {Paths} */
const globalPathCache: any = {}; // 缓存path
/** @type {MoveTimer} */
const pathCacheTimer: any = {}; // 用于记录path被使用的时间，清理长期未被使用的path
/** @type {CreepPaths} */
const creepPathCache: any = {}; // 缓存每个creep使用path的情况
const creepMoveCache = {}; // 缓存每个creep最后一次移动的tick
const emptyCostMatrix = new PathFinder.CostMatrix();
/** @type {CMs} */
const costMatrixCache: any = {}; // true存ignoreDestructibleStructures==true的，false同理
/** @type {{ [time: number]:{roomName:string, avoids:string[]}[] }} */
const costMatrixCacheTimer: any = {}; // 用于记录costMatrix的创建时间，清理过期costMatrix
...
let cacheHitCost = 0;
let cacheMissCost = 0;
const unWalkableCCost = 255;

/***************************************
 *  util functions
 */
const reg1 = /^([WE])([0-9]+)([NS])([0-9]+)$/; // parse得到['E28N7','E','28','N','7']
/**
 *  统一到大地图坐标，平均单次开销0.00005
 * @param {RoomPosition} pos
 */
function formalize(pos: RoomPosition): any {
	const splited = reg1.exec(pos.roomName);
	if (splited && splited.length == 5) {
		return {
			// 如果这里出现类型错误，那么意味着房间名字不是正确格式但通过了parse，小概率事件
 ...
 */
function isEqual(pos1: RoomPosition, pos2: RoomPosition) {
 ...
 * @param {RoomPosition} pos2
 */
function isNear(pos1: RoomPosition, pos2: RoomPosition) {
	if (pos1.roomName == pos2.roomName) {
		// undefined == undefined 也成立
		return -1 <= pos1.x - pos2.x && pos1.x - pos2.x <= 1 && -1 <= pos1.y - pos2.y && pos1.y - pos2.y <= 1;
	} else if (pos1.roomName && pos2.roomName) {
		// 是完整的RoomPosition
		if (pos1.x + pos2.x != 49 && pos1.y + pos2.y != 49) return false; // 肯定不是两个边界点, 0.00003 cpu
		// start
		const splited1 = reg1.exec(pos1.roomName);
		const splited2 = reg1.exec(pos2.roomName);
		if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
			// 统一到大地图坐标
			const formalizedEW =
				(splited1[1] === 'W' ? -splited1[2] : +splited1[2] + 1) * 50 +
				pos1.x -
				(splited2[1] === 'W' ? -splited2[2] : +splited2[2] + 1) * 50 -
				pos2.x;
			const formalizedNS =
				(splited1[3] === 'N' ? -splited1[4] : +splited1[4] + 1) * 50 +
				pos1.y -
				(splited2[3] === 'N' ? -splited2[4] : +splited2[4] + 1) * 50 -
				pos2.y;
 ...
 * @param {RoomPosition} pos2
 */
function inRange(pos1: RoomPosition, pos2: RoomPosition, range: number) {
	if (pos1.roomName == pos2.roomName) {
 ...
 */
function getDirection(fromPos: RoomPosition, toPos: RoomPosition) {
...
const reg2 = /^[WE]([0-9]+)[NS]([0-9]+)$/; // parse得到['E28N7','28','7']
const isHighWay = config.roomNameMoreThan100
	? (roomName: string) => {
		const splited = reg2.exec(roomName)!;
 ...
 * @param {*} ops
 */
function isSameOps(path: MyPath, ops: any) {
	return (
		path.ignoreRoads == !!ops.ignoreRoads &&
		path.ignoreSwamps == !!ops.ignoreSwamps &&
		path.ignoreStructures == !!ops.ignoreDestructibleStructures
	);
}

function hasActiveBodypart(body: BodyPartDefinition[], type: BodyPartConstant) {
	if (!body) {
		return true;
	}
...
function isClosedRampart(structure: AnyOwnedStructure) {
 ...
 */
function isObstacleStructure(room: Room, pos: RoomPosition, ignoreStructures: boolean) {
	const consSite = room.lookForAt(LOOK_CONSTRUCTION_SITES, pos);
	if (0 in consSite && consSite[0].my && obstacles.has((consSite[0] as any).structureType)) {
		// 工地会挡路
		return true;
	}
	const struct = room.lookForAt(LOOK_STRUCTURES, pos);
	for (const s of struct) {
		if (!s.hits || (s instanceof StructureInvaderCore && s.ticksToDeploy)) {
			// 是新手墙或者无敌中的invaderCore
			return true;
		} else if (!ignoreStructures && (obstacles.has((s as any).structureType) || isClosedRampart(s as AnyOwnedStructure))) {
			return true;
		}
	}
	return false;
 ...
 * @param {number} idx
 */
function addObTask(path: MyPath, idx: number) {
	const roomName = path.posArray[idx].roomName;
	//console.log('准备ob ' + roomName);
	for (const obData of observers) {
...
function doObTask() {
	for (const obData of observers) {
		...
		while (queue.length) {
			...
			const ob: any = Game.getObjectById(obData.id);
			if (ob) {
				// console.log('Observer [' + ob.id + ']: ' + roomName);
...
function checkObResult() {
	for (const tick in obTimer) {
		if (+tick < Game.time) {
			delete obTimer[tick];
			continue; // 后面可能还有要检查的
		} else if (+tick == Game.time) {
			for (const result of obTimer[tick]) {
 ...
 * @param {RoomPosition} pos
 */
function generateCostMatrix(room, pos: RoomPosition) {
	...
	const totalStructures = room.find(FIND_STRUCTURES);
	const ignoreRoadPos: RoomObject[] = [].concat(room.find(FIND_SOURCES), room.find(FIND_MINERALS), room.find(FIND_DEPOSITS)); // 修路也没用的墙点
	let x, y, noviceWall, deployingCore, centralPortal;
	let clearDelay = Infinity;
	for (const object of ignoreRoadPos) {
		x = object.pos.x;
		y = object.pos.y;
	...
	if (room.controller && (room.controller.my || room.controller.safeMode)) {
		// 自己的工地不能踩
		for (const consSite of room.find(FIND_CONSTRUCTION_SITES)) {
	...
	for (const s of totalStructures) {
		if (s.structureType == STRUCTURE_INVADER_CORE) {
			...
			// 路的移动力损耗是1，此处设置能寻到墙上的路
			x = s.pos.x;
			y = s.pos.y;
			if (noStructureCostMat.get(x, y) == 0) {
				// 不是在3种无敌建筑或墙中资源上
		...
		} else if (obstacles.has(s.structureType) || isClosedRampart(s)) {
			// HELP：有没有遗漏其他应该设置 noStructureCostMat 的点
			structureCostMat.set(s.pos.x, s.pos.y, unWalkableCCost);
		}
	}
	...
	const avoids: Partial<Record<ExitKey, string>>[] = [];
	if (room.controller && room.controller.owner && !room.controller.my && hostileCostMatrixClearDelay) {
		// 他人房间，删除costMat才能更新被拆的建筑位置
	...
	} else if (noviceWall || deployingCore || centralPortal) {
		// 如果遇到可能消失的挡路建筑，这3种情况下clearDelay才可能被赋值为非Infinity
		if (noviceWall) {
			// 如果看见新手墙
			...
			for (const direction in neighbors) {
				...
				if (status.status == 'closed') {
					...
					const estimateTickToChange = (status.timestamp - new Date().getTime()) / 10000; // 10s per tick
			...
			if (pos) {
				...
				for (const direction in neighbors) {
					if (!(neighbors[direction] in avoidRooms)) {
						...
						if (
							PathFinder.search(pos, exits, {
								maxRooms: 1,
								roomCallback: () => noStructureCostMat
							}).incomplete
						) {
							// 此路不通
							avoidRooms[neighbors[direction]] = 1;
 ...
 * @param {MyPath} path
 */
function generateDirectionArray(path: MyPath) {
	const posArray = path.posArray;
	const directionArray = new Array(posArray.length);
	let incomplete = 0;
	for (let idx = 1; idx in posArray; idx++) {
		if (posArray[idx - 1].roomName in costMatrixCache) {
			// 有costMat，是准确路径，否则需要在有视野时checkRoom()
 ...
 */
function checkRoom(room, path: MyPath, startIdx: number) {
	if (!(room.name in costMatrixCache)) {
		generateCostMatrix(room, path.posArray[startIdx]);
	}
	const thisRoomName = room.name;
	/** @type {CostMatrix} */
	const costMat: CostMatrix = costMatrixCache[thisRoomName][String(path.ignoreStructures)];
	const posArray = path.posArray;
	const directionArray = path.directionArray!;
	let i: number;
	for (i = startIdx; i + 1 in posArray && posArray[i].roomName == thisRoomName; i++) {
		if (costMat.get(posArray[i].x, posArray[i].y) == unWalkableCCost) {
			// 路上有东西挡路
			return false;
		}
...
let temporalAvoidFrom: string, temporalAvoidTo: string;
function routeCallback(nextRoomName: any, fromRoomName: any) {
	// 避开avoidRooms设置了的
	if (nextRoomName in avoidRooms) {
		//console.log('Infinity at ' + nextRoomName);
		return Infinity;
	}
	// if (nextRoomName in Memory.bypassoutRooms) {
	//     return Infinity;
	// }
	return isHighWay(nextRoomName) ? 1 : 1.15;
}
function bypassRouteCallback(nextRoomName: string, fromRoomName: string) {
 ...
 */
function findRoute(fromRoomName: string | Room, toRoomName: string | Room, bypass = false) {
	// TODO 以后跨shard寻路也放在这个函数里
	//console.log('findRoute', fromRoomName, toRoomName, bypass);
	return Game.map.findRoute(fromRoomName, toRoomName, {
		routeCallback: bypass ? bypassRouteCallback : routeCallback
	});
}
...
function bypassHostile(creep: Creep) {
	return !creep.my || creep.memory.dontPullMe;
}
function bypassMy(creep: Creep) {
	return creep.my && creep.memory.dontPullMe;
}
...
function bypassRoomCallback(roomName) {
	...
	// if(this.memory.role )
	// if (roomName in Memory.bypassoutRooms) {
	//     return false;
	// }
	if (roomName == bypassRoomName) {
		// 在findTemporalRoute函数里刚刚建立了costMatrix
	...
	if (userCostCallback) {
		const resultCostMat = userCostCallback(
			roomName,
			roomName in costMatrixCache ? costMat.clone() : new PathFinder.CostMatrix()
		);
...
function findTemporalPath(creep, toPos, ops) {
	let nearbyCreeps;
	if (ops.ignoreCreeps) {
		// 有ignoreCreep，只绕过无法对穿的creep
		nearbyCreeps = creep.pos
			.findInRange(FIND_CREEPS, ops.bypassRange, {
	...
	if (!(creep.room.name in costMatrixCache)) {
		// 这个房间的costMatrix已经被删了
	...
	if (ops.ignoreSwamps) {
		...
		PathFinderOpts.swampCost = ops.swampCost || (ops.flee ? 20 : 10);
	}

	if (creep.pos.roomName != toPos.roomName) {
		// findRoute会导致非最优path的问题
		...
		PathFinderOpts.maxRooms = 1;
		PathFinderOpts.roomCallback = bypassRoomCallback;
	}

	const result = PathFinder.search(creep.pos, { pos: toPos, range: ops.range }, PathFinderOpts).path;
	if (result.length) {
		const creepCache = creepPathCache[creep.name];
		creepCache.path = {
			// 弄个新的自己走，不修改公用的缓存路，只会用于正向走所以也不需要start属性，idx属性会在startRoute中设置
 ...
 * @param {MoveToOpts} ops
 */
function findPath(fromPos: RoomPosition, toPos: RoomPosition, ops: MoveToOpts, noPathState = false) {
	if (!(fromPos.roomName in costMatrixCache) && fromPos.roomName in Game.rooms) {
		// 有视野没costMatrix
	...
	// let myroomparsed = Number((/^[WE]([0-9]+)[NS]([0-9]+)$/.exec(fromPos.roomName)));
	// let disRoomparsed = Number((/^[WE]([0-9]+)[NS]([0-9]+)$/.exec(toPos.roomName)));
	...
	const myroomparsed = /^[WE]([0-9]+)[NS]([0-9]+)$/.exec(fromPos.roomName) as any;
	const disRoomparsed = /^[WE]([0-9]+)[NS]([0-9]+)$/.exec(toPos.roomName) as any;
	/* 计算距离 如果两个房间之间距离过短就不这样做 */
	const enoughDistance = Math.sqrt(
		Math.abs(myroomparsed[1] - disRoomparsed[1]) ** 2 + Math.abs(myroomparsed[2] - disRoomparsed[2]) ** 2
	);
	// console.log(fromPos.roomName, toPos.roomName, '距离测算a', enoughDistance, JSON.stringify(myroomparsed), JSON.stringify(disRoomparsed))
	// enoughDistance > 3.6
	if (fromPos.roomName != toPos.roomName && (enoughDistance > 3 || noPathState)) {
		// findRoute会导致非最优path的问题 isOutmine会判断是否为外矿寻找最优路径
		// console.log('预先测算房间距离')
		route = findRoute(fromPos.roomName, toPos.roomName);
		if (route == ERR_NO_PATH) {
			return { path: [] };
		}
		if (noPathState) {
			PathFinderOpts.maxOps = ops.maxOps ? ops.maxOps * 3 : 6000 + route.length ** 2 * 150; // 跨10room则有4000+10*10*100=14000
			PathFinderOpts.maxRooms = (PathFinderOpts.maxRooms || route.length) + 4;
		} else {
			PathFinderOpts.maxOps = ops.maxOps || 3000 + route.length ** 2 * 75; // 跨10room则有3000+10*10*75=10500
			PathFinderOpts.maxRooms = PathFinderOpts.maxRooms || route.length + 2;
		}
 ...
 */
function addPathIntoCache(newPath: MyPath) {
	if (!newPath.start || !newPath.end) return;
...
function deletePath(path) {
	if (path.start) {
		// 有start属性的不是临时路
		const pathArray = globalPathCache[path.start.x + path.start.y][path.end.x + path.end.y];
		...
		if (combinedX in globalPathCache) {
			for (combinedY = minY; combinedY <= maxY; combinedY++) {
				if (combinedY in globalPathCache[combinedX]) {
					for (const path of globalPathCache[combinedX][combinedY]) {
						// 这个数组应该会很短
						pathCounter++;
						if (
							isNear(path.start, formalFromPos) &&
							isNear(fromPos, path.posArray[1]) &&
							inRange(path.end, formalToPos, ops.range) &&
							isSameOps(path, ops)
						) {
							// 找到路了
...
function setPathTimer(creepCache) {
	if (pathClearDelay) {
		...
		if (
			...
			(startRoomName in Game.rooms && Game.rooms[startRoomName].controller && !Game.rooms[startRoomName].controller!.my)
		) {
			// 跨房路或者敌方房间路
...
function showVisual(creep, toPos, posArray, startIdx, idxStep, visualStyle) {
	...
	const thisRoomName = creep.room.name;
...
function moveOneStep(creep, visualStyle, toPos) {
	...
	testNormal++;
	const t = Game.cpu.getUsed() - startTime;
	if (t > 0.2) {
		// 对穿导致的另一个creep的0.2不计在内
 ...
 * @param {Function} fn
 */
function wrapFn(fn, name: string) {
	return function () {
		...
		const code = fn.apply(this, arguments);
...
function clearUnused() {
	if (pathClearDelay === undefined) return;
	if (Game.time % pathClearDelay == 0) {
		// 随机清一次已死亡creep
		for (const name in creepPathCache) {
			if (!(name in Game.creeps)) {
				delete creepPathCache[name];
			}
		}
	}
	for (const time in pathCacheTimer) {
		if (+time > Game.time) {
			break;
		}
		//console.log('clear path');
		for (const path of pathCacheTimer[time]) {
	...
	for (const time in costMatrixCacheTimer) {
		if (+time > Game.time) {
			break;
		}
		//console.log('clear costMat');
		for (const data of costMatrixCacheTimer[time]) {
			delete costMatrixCache[data.roomName];
			for (const avoidRoomName of data.avoids) {
...
const defaultVisualizePathStyle = {
	fill: 'transparent',
	stroke: '#fff',
	lineStyle: 'dashed',
	strokeWidth: 0.15,
 ...
 */
let [ops, toPos, creepCache, path, idx, posArray, found]: [any, any, any, any, any, any, any] = [
	{},
	undefined,
	undefined,
	undefined,
 ...
 */
function betterMoveTo(firstArg: number | RoomObject, secondArg: number | MoveToOpts, opts: MoveToOpts) {
	if (obTick < Game.time) {
		obTick = Game.time;
		checkObResult();
		doObTask();
	...
	if (typeof toPos.x != 'number' || typeof toPos.y != 'number') {
		...
		//this.say('no tar');
		return ERR_INVALID_TARGET;
	} else if (inRange(this.pos, toPos, ops.range || 1) && !ops.flee) {
		// 已到达
		if (this.memory.lastPos && this.memory.lastPos.time > 6) {
			this.memory.lastPos.time = 0;
	...
	} else if (ops.flee && !inRange(this.pos, toPos, ops.range || 10)) {
		// 已躲开
		return OK;
	}
	ops.range = ops.range || (ops.flee ? 10 : 1);
			...
			if (isEqual(toPos, creepCache.dst) || inRange(posArray[posArray.length - 1], toPos, ops.range)) {
				...
				if (isEqual(this.pos, posArray[idx])) {
					...
					if (
						...
						inRange(this.room.storage.pos, this.pos, coreLayoutRange) &&
						ops.ignoreCreeps
					) {
					...
					if (!path.directionArray[idx + 2]) {
						// 第一次见到该房则检查房间
						if (checkRoom(this.room, path, creepCache.idx)) {
							// 传creep所在位置的idx
							// this.say('新房 可走');
					...
					// 堵路了
					const code = trySwap(this, posArray[idx], ops.bypassHostileCreeps, ops.ignoreCreeps); // 检查挡路creep
					if (code == OK) {
						// 让这个逻辑掉下去
					} else if (code == ERR_INVALID_TARGET) {
						// 是被设置了不可对穿的creep或者敌对creep挡路，临时绕路
						testBypass++;
						ops.bypassRange = ops.bypassRange || 5; // 默认值
						if (typeof ops.bypassRange != 'number' || typeof ops.range != 'number') {
							return ERR_INVALID_ARGS;
						}
						if (findTemporalPath(this, toPos, ops)) {
							// 有路，creepCache的内容会被这个函数更新
							// this.say('开始绕路');
						...
						code == ERR_NOT_FOUND &&
						isObstacleStructure(this.room, posArray[idx], ops.ignoreDestructibleStructures)
					) {
						// 发现出现新建筑物挡路，删除costMatrix和path缓存，重新寻路
	...
	found =
		this.pos.roomName == toPos.roomName
			? findShortPathInCache(formalize(this.pos), formalize(toPos), this.pos, creepCache, ops)
			: findLongPathInCache(formalize(this.pos), formalize(toPos), creepCache, ops);
	if (found) {
		//this.say('cached');
		...
		let result: any = findPath(this.pos, toPos, ops, this.memory.noPathState);
		if (!result.path.length || (result.incomplete && result.path.length == 1)) {
			// 一步也动不了了
			this.say('no path|' + toPos.x + '|' + toPos.y);
			console.log('no path|', JSON.stringify(this.pos) + '|' + JSON.stringify(toPos));
			this.memory.noPathState = true;
			return ERR_NO_PATH;
		} else if (!result.incomplete && ops.flee) {
			// 已逃离到目标位置
			return OK;
		}
		if (this.memory.noPathState) this.memory.noPathState = false;
		result = result.path;
 ...
 */
// function betterMove(target: DirectionConstant | Creep) { }
 ...
 */
// function betterFindClosestByPath(
// 	type: FindConstant,
 ...
 * @param {*} opts
 */
// function findSquadPathTo(toPos: RoomPosition, opts: any) { }
...
observers = observers.reduce((temp, id) => {
	const ob: any = Game.getObjectById(id.id);
...
//@ts-ignore
PowerCreep.prototype.moveTo = betterMoveTo
...
export const SuperMove= {
	...
	setPathClearDelay(number?: number) {
	...
	setHostileCostMatrixClearDelay(number?: number) {
		if (typeof number == 'number' && number > 0) {
	...
	deleteCostMatrix(roomName: string) {
	...
	addAvoidRooms(roomName: string) {
		const splited = reg1.exec(roomName);
		if (splited && splited.length == 5) {
	...
	deleteAvoidRooms(roomName: string) {
		const splited = reg1.exec(roomName);
	...
	addObList(obList: string[]) {
		if (observers.length <= 0) {
			observers = _.uniq(
				obList.reduce((temp: any[], id) => {
					const ob: StructureObserver | null = Game.getObjectById(id as Id<StructureObserver>);
					if (ob && ob.observeRoom! && ob.my) {
						temp.push({id, roomName: ob.room.name, taskQueue: []});
					}
					return temp;
			...
			for (const id of obList) {
				const ob: StructureObserver | null = Game.getObjectById(id as Id<StructureObserver>);
				const existingOb = observers.map((ob) => ob.id);
				if (ob && ob.observeRoom! && ob.my && !existingOb.includes(id)) {
					observers.push({id, roomName: ob.room.name, taskQueue: []});
				}
			}
		}
		console.log(`OB列表更新成功,当前OB房间列表: ${observers.map((ob) => ob.roomName)}`);
	},
	clearObList() {
		observers = [];
	},
	deletePathInRoom(roomName: string) {
		const splited = reg1.exec(roomName);
		if (splited && splited.length == 5) {
			this.deleteCostMatrix(roomName);
			const fromalCentralPos = formalize(new RoomPosition(25, 25, roomName));
					...
					for (combinedY = minY; combinedY <= maxY; combinedY++) {
						if (combinedY in globalPathCache[combinedX]) {
							for (const path of globalPathCache[combinedX][combinedY]) {
								...
								const posArray = path.posArray;
								if (
									posArray[0].roomName == roomName &&
									posArray[posArray.length - 1].roomName == roomName
								) {
									// 是这个房间的路
									deletePath(path);
								}
	...
	addAvoidExits(fromRoomName: string, toRoomName: string) {
		// 【未启用】
		const splited1 = reg1.exec(fromRoomName);
		const splited2 = reg1.exec(toRoomName);
		if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
			avoidExits[fromRoomName]
				? (avoidExits[fromRoomName][toRoomName] = 1)
				: (avoidExits[fromRoomName] = {[toRoomName]: 1});
			return OK;
		} else {
			return ERR_INVALID_ARGS;
	...
	deleteAvoidExits(fromRoomName: string, toRoomName: string) {
		// 【未启用】
		const splited1 = reg1.exec(fromRoomName);
		const splited2 = reg1.exec(toRoomName);
		if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
	...
	print() {
		let text = 'avarageTime\tcalls\tFunctionName';
		for (const fn in analyzeCPU) {
			text += `\n${(analyzeCPU[fn].sum / analyzeCPU[fn].calls).toFixed(5)}\t\t${analyzeCPU[fn].calls}\t\t${fn}`;
		}
		const hitCost = cacheHitCost / testCacheHits;
		const missCost = cacheMissCost / testCacheMiss;
		const missRate = testCacheMiss / (testCacheMiss + testCacheHits);
		text += `\nnormal logical cost: ${(normalLogicalCost / testNormal).toFixed(5)}, total cross rate: ${(
			testTrySwap / analyzeCPU.moveTo.calls
		).toFixed(4)}, total bypass rate:  ${(testBypass / analyzeCPU.moveTo.calls).toFixed(4)}`;
		text += `\nnear storage check rate: ${(testNearStorageCheck / analyzeCPU.moveTo.calls).toFixed(
			4
		)}, near storage cross rate: ${(testNearStorageSwap / testNearStorageCheck).toFixed(4)}`;
		text += `\ncache search rate: ${((testCacheMiss + testCacheHits) / analyzeCPU.moveTo.calls).toFixed(
			4
		)}, total hit rate: ${(1 - missRate).toFixed(4)}, avg check paths: ${(
			pathCounter /
			(testCacheMiss + testCacheHits)
		).toFixed(3)}`;
		text += `\ncache hit avg cost: ${hitCost.toFixed(5)}, cache miss avg cost: ${missCost.toFixed(
			5
		)}, total avg cost: ${(hitCost * (1 - missRate) + missCost * missRate).toFixed(5)}`;
		return text;
	},
	findRoute(fromRoom: string, toRoom: string, bypass?: boolean) {
		return findRoute(fromRoom, toRoom, bypass);
	}
};
